...
export const RESERVED_PLAYER_COLORS = [GENUINE_GOLD_COLOR, EMERALD_RAV_COLOR, CATHARSIS_COLOR, GYDRO_COLOR] as const;
...
export const LOCKED_PLAYER_IDENTITIES: ReadonlyArray<LockedPlayerIdentity> = [
  ...
  },
  {
    ...
    name: GYDRO_NAME,
    shortLabel: 'GydRo',
    title: 'GydRo - reserved hydro',
    aliases: ['gydro', 'руслан', 'ruslan'],
  },
];

export function getLockedPlayerName(color: Color): string | undefined {
  return LOCKED_PLAYER_IDENTITIES.find((identity) => identity.color === color)?.name;
}
