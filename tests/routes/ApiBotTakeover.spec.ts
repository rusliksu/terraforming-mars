...
describe('ApiBotTakeover', () => {
  ...
  it('starts bot takeover from the player page without a shared token', async () => {
  ...
  it('does not classify an originally automated player as a human takeover', async () => {
    const player = TestPlayer.BLACK.newPlayer();
    const game = Game.newInstance('g123456789abc', [player], player, 'spectatorid');
  ...
  it('records surrender separately from reversible bot takeover', async () => {
    ...
    const route = new ApiBotTakeover({
      ...
      stop: (playerId) => {
        ...
        return {gameId: game.id, playerId, pid: 321, startedAtMs: 1, logFile: 'bot.log'};
      },
    });

    scaffolding.url = `/api/bot-takeover?action=surrender&gameId=${game.id}&playerId=${alice.id}`;
    await route.processRequest(scaffolding.req, res, scaffolding.ctx);

    expect(res.statusCode).eq(statusCode.ok);
    expect(game.surrenderedPlayerIds.has(alice.id)).eq(true);
    expect(game.botTakeoverPlayerIds.has(alice.id)).eq(false);
    ...
    expect(auditEvents[0].metadata).deep.eq({action: 'surrender', authorization: 'player'});
  });

  it('does not let stop takeover undo surrender', async () => {
    const player = TestPlayer.BLACK.newPlayer();
    const game = Game.newInstance('g123456789abc', [player], player, 'spectatorid');
