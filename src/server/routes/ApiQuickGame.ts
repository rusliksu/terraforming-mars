...
export class ApiQuickGame extends Handler {
  ...
  private settingsToConfig(settings: Record<string, any>, playerCount: number): NewGameConfig {
    ...
    return {
      ...
      privateHands: settings.privateHands ?? true,
      noEloGame: settings.noEloGame ?? false,
      turnBasedGame: false,
      botGame: false,
      ...
      initialDraftOneWay: settings.initialDraftOneWay === true,
  ...
  public override async get(req: Request, res: Response, ctx: Context): Promise<void> {
    ...
    try {
      ...
      const gameOptions: GameOptions = {
        ...
        turnBasedGame: gameReq.turnBasedGame === true,
      ...
      const game = Game.newInstance(gameId, players, players[firstPlayerIdx], spectatorId, gameOptions, seed, ctx.gameLoader.saveGame.bind(ctx.gameLoader));
