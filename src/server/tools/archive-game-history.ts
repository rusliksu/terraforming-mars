...
const usage = 'Usage: archive-game-history --offline --source files|sqlite --input <offline-D-path> --game <id> --output <private-D-directory>\n' +
...
async function main(): Promise<void> {
  try {
    ...
    let output: string;
    try {
      requireArchive(args.length <= 9 && args.every((arg) => arg.length <= 4096));
      ...
      requireArchive(names.length === 5 && new Set(names).size === 5 && values.offline === true &&
      ...
      selection = {kind: values.source, path: values.input, gameId: values.game, offline: true};
      output = values.output;
    } catch {
      throw new SyntaxError('INVALID_ARGUMENTS');
    }
    const receipt = await exportHistory(new HistorySource(selection), output);
