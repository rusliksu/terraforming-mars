...
try {
    ...
    & ssh.exe $HostAlias "mkdir -p '$remoteUploadRoot'"
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to create remote upload root."
    }

    & scp.exe $tempScript.FullName "${HostAlias}:$remoteScriptPath"
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to upload restore script."
    }
    & scp.exe $archivePath "${HostAlias}:$remoteArchive"
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to upload backup archive."
    }
    & scp.exe $metadataPath "${HostAlias}:$remoteMetadata"
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to upload metadata.json."
    }
    & scp.exe $releasePath "${HostAlias}:$remoteRelease"
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to upload release.json."
    }

    & ssh.exe $HostAlias "chmod 700 '$remoteScriptPath' && bash '$remoteScriptPath'"
    if ($LASTEXITCODE -ne 0) {
        throw "Remote restore failed."
    }
} finally {
    & ssh.exe $HostAlias "rm -rf '$remoteUploadRoot'" | Out-Null
