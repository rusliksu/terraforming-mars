...
export class Executor implements BehaviorExecutor {
  ...
  public execute(behavior: Behavior, player: IPlayer, inputCard: ICard | IGlobalEvent, executionOptions: BehaviorExecutionOptions = {}) {
    ...
    const from: From | undefined = executionOptions.logSource === false ? undefined : (globalEvent ? {globalEvent} : {card});
    ...
    if (behavior.or !== undefined) {
      ...
      for (const subBehavior of behavior.or.behaviors) {
        ...
        const option = new SelectOption(subBehavior.title)
          .andThen(() => {
            this.execute(subBehavior, player, inputCard, executionOptions);
            return undefined;
          });
    ...
    if (behavior.spend !== undefined) {
      ...
      if (spend.megacredits) {
        ...
        })).andThen(() => this.execute(remainder, player, inputCard, executionOptions));
    ...
    if (behavior.addResourcesToAnyCard) {
      const subExecutor = new AddResourcesToAnyCardExecutor(player, card, behavior, ctx, behavior.addResourcesToAnyCard, from);
      subExecutor.execute();
    }
