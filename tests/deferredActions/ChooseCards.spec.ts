...
describe('ChooseCards', () => {
  ...
  it('shows calculated project costs when paying for cards', () => {
    ...
    expect(model.cards.map((card) => ({name: card.name, calculatedCost: card.calculatedCost}))).deep.eq([
      {name: CardName.AQUIFER_PUMPING, calculatedCost: player.getCardCost(aquiferPumping)},
      {name: CardName.IO_MINING_INDUSTRIES, calculatedCost: player.getCardCost(ioMiningIndustries)},
    ]);
