# live-server-card-render-metadata Specification

## Purpose
Зафиксировать, что работающий сервер не строит и не удерживает неиспользуемые деревья `renderData`, а сборочные и тестовые процессы продолжают создавать полные метаданные отображения карт.
## Requirements
